...
Responde en JSON con esta estructura:
{
  ...
  try {
    ...
    // Fallback: intentar sin forzar JSON (texto libre)
    try {
      const apiKey = process.env.GEMINI_API_KEY;
      const res = await fetch(`${GEMINI_URL}?key=${apiKey}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          contents: [{ parts: [{ text: prompt.replace("Responde en JSON con esta estructura:", "Responde de forma directa y concisa en español.").replace(/\{[\s\S]*?\}/m, "") }] }],
      ...
      if (res.ok) {
        const body = await res.json();
        const text = body?.candidates?.[0]?.content?.parts?.[0]?.text || "";
        if (text) return { response: text, suggestions: [], relatedAgents: [] };
      }
    } catch {}
    return {
      response: `Error al procesar tu pregunta: ${err.message}`,
      suggestions: ["Verificar GEMINI_API_KEY", "Revisar status de agentes"],
      relatedAgents: [],
      _fallback: true,
    };
  }
